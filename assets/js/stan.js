...
const roomsData = [
  {
    ...
    size: "35 m²",
    capacity: "max. 2 pers.",
    amenities: ["Free Wi-Fi", "Coffee Maker", "Bathroom with Shower"],
  ...
  },
  {
    ...
    size: "60 m²",
    capacity: "max. 6 pers.",
    amenities: ["Free Wi-Fi", "Mini Kitchen", "Bathroom with Bathtub", "TV"],
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Initialize Flatpickr with range mode on #checkin
  const fp = flatpickr(checkinInput, {
    mode: "range",          // Enable drag-to-select range
    showMonths: 2,          // Show two months for easier range selection
    dateFormat: "Y-m-d",    // Format for form submission compatibility
    minDate: "today",       // Prevent past dates (optional)
    onChange: function(selectedDates, dateStr, instance) {
      if (selectedDates.length === 2) {
        const [start, end] = selectedDates;
        // Set check-in and check-out values in Y-m-d format
  ...
  // Make calendar icons clickable (if they exist in your HTML)
